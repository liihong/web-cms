        ...
        >
          <el-form-item :label="item.COLUMN_CNAME" :prop="item.COLUMN_NAME" style="width:100%">
            ...
            <template style="width:100%" v-else-if="item.PROPERTY_TYPE == '2'">
              <!--下拉选择-->
              <el-select
                @change="$emit('selectChange',formData)"
                style="width:100%"
                clearable
                filterable
              ...
              >
                <el-option
                  v-for="(item,key) in dropDownListData[item.COLUMN_NAME]"
                  :key="key"
...
export default {
  ...
  methods: {
    ...
    getForeingKeyData(tableId, column_name) {
      resMgrServices.getForeingKeyListData(this.queryData).then(res => {
      ...
      this.getConfig().then(() => {
        this.columnData.forEach(item => {
          ...
          if (item.ISMUST == "1") {
            ...
            this.$set(this.rules, item.COLUMN_NAME, [
              {
                required: true,
                message: `请输入${item.COLUMN_CNAME}`,
                trigger: "blur"
              }
            ]);
          }
