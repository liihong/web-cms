...
module.exports = class extends Base {
  ...
  async addTableResAction() {
    try {
      ...
      const affectedRows = await this.model('resource_table').add(addInfo);
  ...
  async editTableResAction() {
    try {
      ...
      const table_id = updateInfo.TABLE_ID;
      const affectedRows = await this.model('resource_table').where({
        table_id
      }).update(updateInfo);
      return this.success(affectedRows);
    } catch (ex) {
  ...
  async deleteTableResAction() {
    try {
      const updateInfo = this.post();
      const affectedRows = await this.model('resource_table').where(updateInfo).delete();
      return this.success(affectedRows);
    } catch (ex) {
      return this.fail(ex);
    }
  ...
  async createTableAction() {
    try {
      ...
      const tableInfo = await this.model('resource_table').getTableInfo(tableId);
      const tableColumns = await this.model('resource_table_column').getColumnList(tableId);
      ...
      const dropTbale = 'drop table ' + tableInfo.table_name;
      const delFlag = await this.model().execute(dropTbale);

      const flag = await this.model().execute(sql);
  ...
  async loadTableAction() {
    try {
      ...
      const tableConfig = await this.model('information_schema.columns ').field('COLUMN_NAME,COLUMN_COMMENT COLUMN_CNAME,DATA_TYPE')
        .where({
          table_name: tableInfo.table_name
        }).select();
      const insertData = tableConfig.map((item, i) => {
      ...
      const deleteInfo = await this.model('resource_table_column').where({table_id: ['=', tableId]}).delete();
