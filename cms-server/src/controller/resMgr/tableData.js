...
module.exports = class extends Base {
  ...
  async getTableConfigAction() {
    ...
    table.primary_key = displayColumn.find(v => v.property_type === '10')['column_name'] || 'id';
    return this.success({
      table: table,
  ...
  async queryTableDataAction() {
    ...
    displayColumn.map(item => {
      switch (item.property_type) {
        ...
        case '2': // 有外键关系,需要翻译
          displayColumnArr.push(item.column_name);
          displayColumnArr.push(`(SELECT NAME FROM (${item.type_sql}) tras WHERE tras.id=${item.column_name}) ${item.column_name}_TEXT`);
          break;
        case '4': // 字段数据
